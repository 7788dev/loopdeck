...
class Ajax extends Common
{
	...
	public function tieba($act = null)
	{
		...
		/* 百度贴吧功能已停用
		if ($act === "add") {
			$cookie = trim((string)Request::post("cookie", ""));
			if ($cookie === "" || !str_contains($cookie, "BDUSS=")) {
				return resultJson(0, "Cookie 至少需要包含 BDUSS");
			}
			$client = new \tieba\TiebaHelper(["cookie" => $cookie]);
			$account = $client->getAccountInfo();
			if ($client->cookiezt || empty($account["uid"])) {
				return resultJson(0, "贴吧 Cookie 验证失败或已失效");
			}
			return Accounts::add("tieba", $account["uid"], $account);
		}
		return $this->accountAction("tieba", $act);
		*/
	}

	public function mihoyo($act = null)
	{
		return response('Not Found', 404);
		...
		if ($act === "add") {
			$cookie = trim((string)Request::post("cookie", ""));
			if ($cookie === "") {
				return resultJson(0, "请先输入米游社 Cookie");
			}
			$client = new \mihoyo\Mihoyo(["cookie" => $cookie]);
			$account = $client->getAccountSummary();
			if ($client->cookiezt || empty($account["ltuid"])) {
				return resultJson(0, "米游社 Cookie 验证失败或缺少账号标识");
			}
			return Accounts::add("mihoyo", $account["ltuid"], $account);
		}
		return $this->accountAction("mihoyo", $act);
		*/
	}
