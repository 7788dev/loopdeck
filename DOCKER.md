...
当前代码已精简为单站点，镜像不再安装 Node.js。升级继续使用主站（`web_id = 1`）和原有数据卷；其他旧站点的登录、任务和推送停止处理，已下架商品的订单和卡密不再开通权益。不会自动删除历史数据库记录，也不会把其他站点用户合并为主站用户。首次安装使用精简后的表结构，已有安装无需执行删表或删字段操作。
...
从 v1.2.3 起，回滚镜像取自正在运行的应用容器，不受镜像标签移动影响。更新器自身的重建交给一个临时辅助容器执行，避免重建命令随旧更新器一起结束。辅助容器使用已校验的本地镜像、只读继承挂载、独立进程和 64 MB 内存限制，无外部网络，完成后自动删除；没有新增常驻服务。若辅助容器启动失败，现有更新器会按重试间隔再次尝试。

v1.2.2 及更早版本的更新器可能在重建自身时停止。首次修复这类部署时，在宿主机项目目录执行 `docker compose pull updater` 和 `docker compose up -d --no-build --no-deps --pull never updater`，由新版更新器继续检查和升级应用；这两个命令不重建数据库，也不删除持久化卷。
