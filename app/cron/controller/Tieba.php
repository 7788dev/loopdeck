...
use Exception;
use tieba\TiebaHelper as TiebaAPI;
use think\facade\Request;

class Tieba extends Common
{
    public function index()
    {
        ...
        $urls = [];
        $vip_expired_userIds = [];
        $jobs = Jobs::getUnexecutedList('tieba'); // 获取未执行任务列表
        if (count($jobs) == 0) {
        ...
        foreach ($jobs as $job) {
            ...
            if($user == null){
                Jobs::delJob('tieba',$job['user_id']);
                continue;
            }
            $task = Tasks::where('type', '=', 'tieba')->where('execute_name', '=', $job['do'])->find();
            $account = Accounts::where('type', '=', 'tieba')->where('user_id', '=', $job['user_id'])->find();
            if ($account == null) {
            ...
            if ($task['vip'] == 1 && strtotime($user['vip_end'] ?? '') < time()) {  // 判断会员功能、用户会员是否过期
                $this->vipExpired('tieba', $user['uid'], $job['user_id']); // 会员过期处理
    ...
    public function execute($do)
    {
        ...
        if (isset($data['runkey']) && $data['runkey'] == RUN_KEY) {
            $tieba = new TiebaAPI($data);
            $execute = $tieba->{$do}();
            if ($tieba->cookiezt) {
                $account = Accounts::where('type', '=', 'tieba')->where('user_id', '=', $data['user_id'])->find();
                $user = Users::where('uid', '=', $account['uid'])->find();
                $this->accountInvalid('tieba', $user, $data['user_id']); // 账号失效处理
            } else {
                foreach ($tieba->msg as $message) {
                    TaskLogs::operateExecuteLog('tieba', $data['user_id'], $do, $message); // 写入运行日志
                }
            }
        } else {
    ...
    private function getExecuteUrl($user_id, $do)
    {
        ...
        $account = Accounts::where('type', '=', 'tieba')->where('user_id', '=', $user_id)->find();
        $account_info = unserialize($account['data']);

        $account_data = http_build_query($account_info);

        $url = get_Domain() . "cron/tieba/{$do}?user_id={$account_info['uid']}" . $job_config . "&" . $account_data . "&runkey=" . RUN_KEY;
        return $url;
    }
